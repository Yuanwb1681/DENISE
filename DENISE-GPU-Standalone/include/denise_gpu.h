...
float *denise_gpu_device_vector(unsigned long elements);
void *denise_gpu_device_alloc(unsigned long bytes);
int denise_gpu_zero_matrix(float **matrix,int ylo,int yhi,int xlo,int xhi);
...
int denise_gpu_finish_shot_psv(float **g_vp,float **g_vs,float **g_rho,
 float **g_lam,float **g_mu,float **g_rho_s,float **sum_vp,float **sum_vs,
 float **sum_rho,float **ws,float **we,float **vp,float **vs,float **rho,
 int nx,int ny,int idx,int idy,int invmat,int grad_form,int iter,
 int inv_vp_iter,int inv_vs_iter,int inv_rho_iter,float dt,float c_vp,
 float c_vs,float c_rho,float dh,int global_nx);
int denise_gpu_residual_l2(float **observed,float **synthetic,float **residual,
 int ntr,int ns,int grad_form,int mode,int accumulate_l2,float dt,
 double initial_l2,double *result_l2);
int denise_gpu_filter_traces(float **data,int ntr,int ns,int order,float fc,
 float dt,int highpass);
int denise_gpu_model_stats_psv(float **g_vp,float **g_vs,float **g_rho,
 float **vp,float **vs,float **rho,int nx,int ny,float stats[6]);
int denise_gpu_model_update_psv(float **g_vp,float **g_vs,float **g_rho,
 float **vp,float **vs,float **rho,float **vp_new,float **vs_new,float **rho_new,
 int nx,int ny,int invmat1,int commit,float eps_vp,float eps_vs,float eps_rho,
 float vp_lo,float vp_hi,float vs_lo,float vs_hi,float rho_lo,float rho_hi);
int denise_gpu_scale_copy_psv(float **vp,float **vs,float **rho,float **vp_copy,
 float **vs_copy,float **rho_copy,int nx,int ny,int idx,int idy,float vp_scale,
 float vs_scale,float rho_scale);
int denise_gpu_pack_pcg_psv(float *vec,float **vp,float **vs,float **rho,
 int nx,int ny,int idx,int idy,int unpack);
int denise_gpu_pcg_update(float *current,float *previous,float *direction,
 int classes,int elements_per_class,int beta_method,float *beta);
int denise_gpu_lbfgs_update(float *y,float *s,float *rho,float *alpha,float *q,
 float *r,int pointer,int history,int vector_length);
