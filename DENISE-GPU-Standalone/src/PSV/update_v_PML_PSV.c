...
void update_v_PML_PSV(int nx1, int nx2, int ny1, int ny2, int nt,
	 ...
	 * ------------------------------------------------------------ */
#ifdef DENISE_USE_CUDA
	if(!getenv("DENISE_GPU_DISABLE_PSV_V")){
	 if(denise_gpu_update_v_psv(nx1,nx2,ny1,ny2,vx,vy,vxp1,vyp1,sxx,syy,sxy,
	 rip,rjp,hc,K_x,a_x,b_x,K_x_half,a_x_half,b_x_half,K_y,a_y,b_y,K_y_half,
	 a_y_half,b_y_half,psi_sxx_x,psi_syy_y,psi_sxy_y,psi_sxy_x,sw,FDORDER,FW,
	 FREE_SURF,BOUNDARY,NPROCX,NPROCY,POS[1],POS[2],GRAD_FORM,DT,DH)!=0)
	 err("CUDA PSV velocity update failed");
	 if(denise_gpu_vsource_psv(nt,vx,vy,srcpos_loc,signals,signals1,nsrc,sw,QUELLTYPB)!=0)
	  err("CUDA PSV force source injection failed");
	 return;
	}
#endif
