...
float calc_mat_change_test_PSV(float  **  waveconv, float  **  waveconv_rho, float  **  waveconv_u, float  **  rho, float  **  rhonp1, float **  pi, float **  pinp1, float **  u, float **  unp1, int iter, 
	...
	if (!getenv("DENISE_GPU_DISABLE_MODEL_UPDATE") &&
	    denise_gpu_model_stats_psv(waveconv,waveconv_u,waveconv_rho,pi,u,rho,NX,NY,gpu_stats)==0) {
		pimax=gpu_stats[0]; gradmax=gpu_stats[1]; umax=gpu_stats[2];
		gradmax_u=gpu_stats[3]; rhomax=gpu_stats[4]; gradmax_rho=gpu_stats[5];
		gpu_fast=1; goto model_stats_ready;
	}
...
model_stats_ready:
	...
	if (gpu_fast && denise_gpu_model_update_psv(waveconv,waveconv_u,waveconv_rho,
	    pi,u,rho,pinp1,unp1,rhonp1,NX,NY,INVMAT1,itest==0,EPSILON,EPSILON_u,
	    EPSILON_rho,VPLOWERLIM,VPUPPERLIM,VSLOWERLIM,VSUPPERLIM,
	    RHOLOWERLIM,RHOUPPERLIM)==0) goto model_update_ready;
