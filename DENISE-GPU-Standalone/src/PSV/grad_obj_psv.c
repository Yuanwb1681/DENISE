				   ...
				   MPI_Request *req_send, MPI_Request *req_rec)
{
	...
	extern int INVMAT1, GRAD_FORM, INV_VP_ITER, INV_VS_ITER, INV_RHO_ITER, NXG;
...
#ifdef DENISE_USE_CUDA
		if (EPRECOND == 3 && !SWS_TAPER_CIRCULAR_PER_SHOT && !RTM_SHOT &&
		    !getenv("DENISE_GPU_DISABLE_FINISH_SHOT") &&
		    denise_gpu_finish_shot_psv((*fwiPSV).waveconv_shot,
		      (*fwiPSV).waveconv_u_shot,(*fwiPSV).waveconv_rho_shot,
		      (*fwiPSV).waveconv_lam,(*fwiPSV).waveconv_mu,
		      (*fwiPSV).waveconv_rho_s,(*fwiPSV).waveconv,
		      (*fwiPSV).waveconv_u,(*fwiPSV).waveconv_rho,Ws,We,
		      (*matPSV).ppi,(*matPSV).pu,(*matPSV).prho,NX,NY,IDX,IDY,
		      INVMAT1,GRAD_FORM,iter,INV_VP_ITER,INV_VS_ITER,INV_RHO_ITER,
		      DT,C_vp,C_vs,C_rho,DH,NXG)==0)
			goto psv_shot_gradient_done;
#endif
