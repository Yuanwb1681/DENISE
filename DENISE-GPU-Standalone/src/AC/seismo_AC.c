...
#ifdef DENISE_USE_CUDA
	if (!getenv("DENISE_GPU_DISABLE_AC_SEISMO")) {
	if(denise_gpu_seismo_ac(lsamp,ntr,recpos,sectionvx,sectionvy,sectionp,
	 sectioncurl,sectiondiv,vx,vy,p,pi,u,rho,hc,SEISMO,FDORDER,INVMAT1,DT,DH)!=0)
	 err("CUDA acoustic receiver sampling failed");
	return;
	}
#endif
