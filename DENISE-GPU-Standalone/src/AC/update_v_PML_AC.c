...
#ifdef DENISE_USE_CUDA
	if (!getenv("DENISE_GPU_DISABLE_AC_V")) {
	if (denise_gpu_update_v_acoustic(nx1,nx2,ny1,ny2,vx,vy,vxp1,vyp1,p,
		rip,rjp,hc,K_x_half,a_x_half,b_x_half,K_y_half,a_y_half,b_y_half,
		psi_p_x,psi_p_y,sw,FDORDER,FW,FREE_SURF,BOUNDARY,NPROCX,NPROCY,
		POS[1],POS[2],GRAD_FORM,DT,DH) != 0)
		err("CUDA acoustic velocity update failed");
	goto inject_sources;
	}
#endif
