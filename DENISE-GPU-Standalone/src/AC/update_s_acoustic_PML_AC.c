...
#ifdef DENISE_USE_CUDA
	if (!getenv("DENISE_GPU_DISABLE_AC_S")) {
	if (denise_gpu_update_s_acoustic(nx1, nx2, ny1, ny2, vx, vy, ux, p,
		pi, rho, hc, K_x, a_x, b_x, K_y, a_y, b_y, psi_vxx, psi_vyy,
		mode, FDORDER, INVMAT1, FW, FREE_SURF, BOUNDARY, NPROCX, NPROCY,
		POS[1], POS[2], GRAD_FORM, DT, DH) != 0)
		err("CUDA acoustic stress update failed");
	return;
	}
#endif
