...
void ac(struct waveAC *waveAC, struct waveAC_PML *waveAC_PML, struct matAC *matAC, struct fwiPSV *fwiPSV, struct mpiPSV *mpiPSV, 
	...
	if((nt==hin1)&&(mode==0)&&(MODE>0)){
		...
		if(!getenv("DENISE_GPU_DISABLE_FWI_AC")){
		 if(denise_gpu_fwi_store_ac((*waveAC).pvxp1,(*waveAC).pvyp1,(*waveAC).p,
		   (*waveAC).ux,(*fwiPSV).forward_prop_rho_x,(*fwiPSV).forward_prop_rho_y,
		   (*fwiPSV).forward_prop_x,NX,NY,IDXI,IDYI,imat,GRAD_FORM)!=0)
		   err("CUDA AC forward wavefield store failed");
		 imat+=NXNYI;imat1+=NXNYI;goto ac_store_done;
		}
#endif
...
#ifdef DENISE_USE_CUDA
		    if(!getenv("DENISE_GPU_DISABLE_FWI_AC")){
		     if(denise_gpu_fwi_corr_ac((*waveAC).pvxp1,(*waveAC).pvyp1,(*waveAC).p,
		       (*fwiPSV).waveconv_shot,(*fwiPSV).waveconv_rho_shot,
		       (*fwiPSV).forward_prop_rho_x,(*fwiPSV).forward_prop_rho_y,
		       (*fwiPSV).forward_prop_x,NX,NY,IDXI,IDYI,imat)!=0)
		       err("CUDA AC gradient correlation failed");
		     imat+=NXNYI;goto ac_corr_done;
		    }
#endif
		    for (i=1;i<=NX;i=i+IDXI){   
			for (j=1;j<=NY;j=j+IDYI){ 
