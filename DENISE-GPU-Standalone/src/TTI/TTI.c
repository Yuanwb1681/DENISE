...
void TTI(struct wavePSV *wavePSV, struct wavePSV_PML *wavePSV_PML, struct matTTI *matTTI, struct fwiPSV *fwiPSV, struct mpiPSV *mpiPSV, 
	...
	if((nt==hin1)&&(mode==0)&&(MODE>0)){
		...
		if(!getenv("DENISE_GPU_DISABLE_FWI_ANISO")){
		 if(denise_gpu_fwi_store_psv((*wavePSV).pvxp1,(*wavePSV).pvyp1,
		   ...
		   imat,GRAD_FORM)!=0) err("CUDA TTI forward wavefield store failed");
		 imat+=NXNYI;imat1+=NXNYI;imat2+=NXNYI;goto tti_store_done;
		}
#endif
...
#ifdef DENISE_USE_CUDA
		    if(!getenv("DENISE_GPU_DISABLE_FWI_ANISO")){
		     if(denise_gpu_fwi_corr_aniso((*wavePSV).pvxp1,(*wavePSV).pvyp1,
		       (*wavePSV).psxx,(*wavePSV).psyy,(*fwiPSV).waveconv_shot,
		       (*fwiPSV).waveconv_rho_shot,(*fwiPSV).forward_prop_rho_x,
		       (*fwiPSV).forward_prop_rho_y,(*fwiPSV).forward_prop_x,
		       (*fwiPSV).forward_prop_y,NX,NY,IDXI,IDYI,imat)!=0)
		       err("CUDA TTI gradient correlation failed");
		     imat+=NXNYI;goto tti_corr_done;
		    }
#endif
		    for (i=1;i<=NX;i=i+IDXI){   
			for (j=1;j<=NY;j=j+IDYI){ 
