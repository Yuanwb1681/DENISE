...
extern "C" float *denise_gpu_device_vector(unsigned long elements)
{
    float *ptr = NULL;
    size_t bytes = ((size_t)elements + 1u) * sizeof(float);
    cudaError_t e = cudaMalloc((void **)&ptr, bytes);
    if (e != cudaSuccess) {
        fprintf(stderr,"DENISE GPU cudaMalloc(%zu) failed: %s\n",bytes,cudaGetErrorString(e));
        return NULL;
    }
    e = cudaMemset(ptr, 0, bytes);
    if (e != cudaSuccess) { cudaFree(ptr); return NULL; }
    return ptr;
}

extern "C" void *denise_gpu_device_alloc(unsigned long bytes)
{
    void *ptr=NULL;cudaError_t e=cudaMalloc(&ptr,(size_t)bytes);
    if(e!=cudaSuccess){fprintf(stderr,"DENISE GPU cudaMalloc(%lu) failed: %s\n",bytes,cudaGetErrorString(e));return NULL;}
    cudaMemset(ptr,0,(size_t)bytes);return ptr;
}

extern "C" int denise_gpu_zero_matrix(float **m,int ylo,int yhi,int xlo,int xhi)
{
    size_t rows=(size_t)(yhi-ylo+1),cols=(size_t)(xhi-xlo+1);
    cudaError_t e=cudaMemsetAsync(m[ylo]+xlo,0,rows*cols*sizeof(float));
    if(e!=cudaSuccess){fprintf(stderr,"DENISE GPU matrix zero failed: %s\n",cudaGetErrorString(e));return -1;}return 0;
}
