...
#ifdef DENISE_USE_CUDA
	if(!getenv("DENISE_GPU_DISABLE_PSV_SOURCE")){
	 if(denise_gpu_psource_psv(nt,NT,DT,sxx,syy,srcpos_loc,signals,nsrc,sw,QUELLTYPB)!=0)
	  err("CUDA PSV source injection failed"); return;
	}
#endif
