...
#ifdef DENISE_USE_CUDA
	v=(float *)denise_gpu_managed_alloc((unsigned long)((nh-nl+1+NR_END)*sizeof(float)));
#else
...
#ifdef DENISE_USE_CUDA
	v=(int *)denise_gpu_managed_alloc((unsigned long)((nh-nl+1+NR_END)*sizeof(int)));
#else
...
#ifdef DENISE_USE_CUDA
	m[nrl]=(float *)denise_gpu_managed_alloc((unsigned long)((nrow*ncol+NR_END)*sizeof(float)));
#else
...
float **device_matrix(int nrl,int nrh,int ncl,int nch){
    int i,nrow=nrh-nrl+1,ncol=nch-ncl+1;float **m;
    m=(float **)malloc((size_t)((nrow+NR_END)*sizeof(float *)));
    if(!m)err("allocation failure 1 in device_matrix()");m+=NR_END;m-=nrl;
    m[nrl]=(float *)denise_gpu_device_alloc((unsigned long)((nrow*ncol+NR_END)*sizeof(float)));
    if(!m[nrl])err("allocation failure 2 in device_matrix()");m[nrl]+=NR_END;m[nrl]-=ncl;
...
int **imatrix(int nrl, int nrh, int ncl, int nch){
	...
	/* allocation rows and set pointers to them */
#ifdef DENISE_USE_CUDA
	m[nrl]=(int *)denise_gpu_managed_alloc((unsigned long)((nrow*ncol+NR_END)*sizeof(int)));
#else
...
#ifdef DENISE_USE_CUDA
	t[nrl][ncl]=(float *)denise_gpu_managed_alloc((unsigned long)((nrow*ncol*ndep+NR_END)*sizeof(float)));
#else
...
#ifdef DENISE_USE_CUDA
	denise_gpu_managed_free((FREE_ARG) (v+nl-NR_END));
#else
...
#ifdef DENISE_USE_CUDA
	denise_gpu_managed_free((FREE_ARG) (m[nrl]+ncl-NR_END));
#else
...
#ifdef DENISE_USE_CUDA
	denise_gpu_managed_free((FREE_ARG) (t[nrl][ncl]+ndl-NR_END));
#else
