...
void LBFGS(int iter, float * y_LBFGS, float * s_LBFGS, float * rho_LBFGS, float * alpha_LBFGS, float * q_LBFGS, float * r_LBFGS, float * beta_LBFGS, int LBFGS_pointer, int NLBFGS, int NLBFGS_vec){
	...
	if(iter>1){
	     if (getenv("DENISE_GPU_ENABLE_LBFGS_RECURSION") &&
	         !getenv("DENISE_GPU_DISABLE_ITERATION_OPS") &&
	         denise_gpu_lbfgs_update(y_LBFGS,s_LBFGS,rho_LBFGS,alpha_LBFGS,
	          q_LBFGS,r_LBFGS,LBFGS_pointer,NLBFGS,NLBFGS_vec)==0) return;
