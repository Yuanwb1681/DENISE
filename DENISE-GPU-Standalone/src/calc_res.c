...
#ifdef DENISE_USE_CUDA
/* Fast production path used by the Marmousi workflow. Filtering and SU input
 * have already completed before this function; form the time-reversed adjoint
 * source and objective on the GPU. */
if(LNORM==2 && !TRKILL && !NORMALIZE && !TIMEWIN && !OFFSET_MUTE &&
   !N_ORDER && COMP_WEIGHT==0 && !getenv("DENISE_GPU_DISABLE_RESIDUAL") &&
   denise_gpu_residual_l2(sectiondata,section,sectiondiff,ntr,ns,GRAD_FORM,
     MODE,swstestshot,DT,L2,&L2)==0) return L2;
#endif
