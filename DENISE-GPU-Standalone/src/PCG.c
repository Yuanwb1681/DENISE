...
void PCG(float * PCG_new, float * PCG_old, float * PCG_dir, int PCG_class){
	...
	if (!getenv("DENISE_GPU_DISABLE_ITERATION_OPS")) {
		int n=((NX+IDX-1)/IDX)*((NY+IDY-1)/IDY);
		if (denise_gpu_pcg_update(PCG_new,PCG_old,PCG_dir,PCG_class,n,
		                          PCG_BETA,beta+1)==0) {
			if(MYID==0) for(k=1;k<=PCG_class;k++)
				printf("\n\n parameter class = %d \t beta = %e\n",k,beta[k]);
			free_vector(beta,1,PCG_class); return;
		}
	}
