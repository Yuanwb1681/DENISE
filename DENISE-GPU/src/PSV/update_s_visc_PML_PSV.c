...
void update_s_visc_PML_PSV(int nx1, int nx2, int ny1, int ny2,
	...
	if(!getenv("DENISE_GPU_DISABLE_PSV_VISCO_S")){
		if(denise_gpu_update_s_visc_psv(nx1,nx2,ny1,ny2,vx,vy,ux,uy,uxy,
			sxx,syy,sxy,r,p,q,fipjp,f,g,bip,bjm,cip,cjm,d,e,dip,hc,K_x,a_x,b_x,
			K_x_half,a_x_half,b_x_half,K_y,a_y,b_y,K_y_half,a_y_half,b_y_half,
			psi_vxx,psi_vyy,psi_vxy,psi_vyx,mode,FDORDER,FW,FREE_SURF,BOUNDARY,
			NPROCX,NPROCY,POS[1],POS[2],GRAD_FORM,DT,DH,L)!=0)
			err("CUDA viscoelastic P-SV stress update failed");
		return;
	}
#endif
