...
#ifdef DENISE_USE_CUDA
	if(!getenv("DENISE_GPU_DISABLE_PSV_S")){
	 if(denise_gpu_update_s_psv(nx1,nx2,ny1,ny2,vx,vy,ux,uy,uxy,sxx,syy,sxy,
	 pi,u,uipjp,rho,hc,K_x,a_x,b_x,K_x_half,a_x_half,b_x_half,K_y,a_y,b_y,
	 K_y_half,a_y_half,b_y_half,psi_vxx,psi_vyy,psi_vxy,psi_vyx,mode,FDORDER,
	 INVMAT1,FW,FREE_SURF,BOUNDARY,NPROCX,NPROCY,POS[1],POS[2],GRAD_FORM,DT,DH)!=0)
	 err("CUDA PSV stress update failed"); return;
	}
#endif
