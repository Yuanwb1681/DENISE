	...
	if((nt==hin1)&&(mode==0)&&(MODE>0)){
		...
		if(!getenv("DENISE_GPU_DISABLE_FWI_ANISO")){
		 if(denise_gpu_fwi_store_psv((*wavePSV).pvxp1,(*wavePSV).pvyp1,
		   ...
		   imat,GRAD_FORM)!=0) err("CUDA VTI forward wavefield store failed");
		 imat+=NXNYI;imat1+=NXNYI;imat2+=NXNYI;goto vti_store_done;
		}
#endif
		...
		imat=((NXNYI*(NTDTINV)) - hin*NXNYI)+1;
		    ...
		    if(!getenv("DENISE_GPU_DISABLE_FWI_ANISO")){
		     if(denise_gpu_fwi_corr_aniso((*wavePSV).pvxp1,(*wavePSV).pvyp1,
		       ...
		       err("CUDA VTI gradient correlation failed");
		     imat+=NXNYI;goto vti_corr_done;
		    }
#endif
		    for (i=1;i<=NX;i=i+IDXI){   
			for (j=1;j<=NY;j=j+IDYI){ 
