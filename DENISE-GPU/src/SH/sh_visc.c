...
void sh_visc(struct waveSH *waveSH, struct waveSH_PML *waveSH_PML, struct matSH *matSH, struct fwiSH *fwiSH, struct mpiPSV *mpiPSV, 
	...
	if((nt==hin1)&&(mode==0)&&(MODE==1||MODE==2)){
	  ...
	  /* store forward wavefields for time-domain inversion and RTM */
		...
		if(!getenv("DENISE_GPU_DISABLE_FWI_SH")){
		 if(denise_gpu_fwi_store_sh((*waveSH).pvzp1,(*waveSH).psxz,(*waveSH).psyz,
		   (*waveSH).uzx,(*waveSH).uz,(*fwiSH).forward_prop_rho_z,
		   (*fwiSH).forward_prop_sxz,(*fwiSH).forward_prop_syz,NX,NY,IDXI,IDYI,
		   imat2,GRAD_FORM)!=0) err("CUDA visco SH forward wavefield store failed");
		 imat2+=NXNYI;goto shv_store_done;
		}
#endif
...
#ifdef DENISE_USE_CUDA
		    if(!getenv("DENISE_GPU_DISABLE_FWI_SH")){
		     if(denise_gpu_fwi_corr_sh((*waveSH).pvzp1,(*waveSH).psxz,(*waveSH).psyz,
		       (*fwiSH).waveconv_rho_shot,(*fwiSH).waveconv_u_shot,
		       (*fwiSH).forward_prop_rho_z,(*fwiSH).forward_prop_sxz,
		       (*fwiSH).forward_prop_syz,(*matSH).prho,(*matSH).pu,NX,NY,IDXI,IDYI,
		       imat,INVMAT1)!=0) err("CUDA visco SH gradient correlation failed");
		     imat+=NXNYI;goto shv_corr_done;
		    }
#endif
		    for (i=1;i<=NX;i=i+IDXI){   
			for (j=1;j<=NY;j=j+IDYI){ 
